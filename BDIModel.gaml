...
global {
    ...
    // ---------------------------------------------------------- CONTROL TEMPORAL ---------------------------------------------------------
    float step <- 1#s;
	float max_time <- 3600#s; // [CONFIG] Duración de la simulación 1 hora - 3600s.
	float hora_init <- 8.00; // [CONFIG] Hora de inicio de la simulación de [00:00 a 23:00].
	float init <- hora_init * 3600;
	bool ic_flag <- false; // [CONFIG]
	bool dyna <- false; // [CONFIG]
	
	int n_simulaciones <- 0; // [CONFIG]
	...
	// ----------------------------------------------------------- CONTROL LOGS ------------------------------------------------------------	
	bool bus_console_logs <- false; // [CONFIG]
	bool tests_console_logs <- false; // [CONFIG]
    bool events_console_logs <- false; // [CONFIG]
    bool passengers_console_logs <- false; // [CONFIG]
    bool dynamic_analysis_console_logs <- false; // [CONFIG]
	...
	list<bus_stop> route_dyna;
	list stops_dyna <- ['07304', '06232', '06236', '11858', '20611'];
	
	list<bus_stop> route_dynb;
	list stops_dynb <- ['20611', '06242', '06250', '06230'];
	
	list all_routes <- (stops_651A + stops_651B + stops_652A + stops_652B + stops_l1 + stops_l2 + stops_dyna + stops_dynb);
    ...
    int frequency_passengers <- 1201#s; // // [CONFIG] 20 minutos
    ...
    init {
        t_inicial <- #now;
	   	...
	   	if hora_init = 8 { prefix <- "BDI_01";}
	    else if hora_init = 11 { prefix <- "BDI_02";}
	    else if hora_init = 14 { prefix <- "BDI_03";}
		...
		route_dyna <- create_route(stops_dyna);
		route_dynb <- create_route(stops_dynb);
		...
		if ic_flag {
			...
			frequency_651 <- 300#s; // [CONFIG] 5 minutos
			frequency_652 <- 600#s; // [CONFIG] 10 minutos
			frequency_l <- 600#s; // [CONFIG] 10 minutos	
		} else {
			frequency_651 <- 600#s; // [CONFIG] 10 minutos
			frequency_652 <- 900#s; // [CONFIG] 15 minutos
			frequency_l <- 900#s; // [CONFIG] 15 minutos
		}
		 
		if dyna {
			prefix <- prefix + "_DYNA";
			
			frequency_dyna <- 600#s; // [CONFIG] 15 minutos
		}
    ...
    reflex create_buses when: !fin_simulacion {    	
        ...
        if dyna and (time - time_dyna >= frequency_dyna) {
			create bus with: [ruta: route_dyna, hora_inicio: to_military_time(time), linea: "DYNA", location: start_point_madrid.location] number: 1;
            create bus with: [ruta: route_dynb, hora_inicio: to_military_time(time), linea: "DYNB", location: start_point_gp.location] number: 1;
            time_dyna <- time;
        }
    }
	...
	reflex passengers when: (mod(cycle, frequency_passengers) = 0) and !fin_simulacion {
		...
		total_passangers <- total_passangers + generados;
		
		write "-----------------------------------------------------------------------------";
		write "[LOGS]: Ciclo " + time + ", Hora: " + to_military_time(init + time) + ", Pasajeros generados: " + generados + ", Total: "+ total_passangers;
		write "-----------------------------------------------------------------------------" + "\n";	
	} 
	...
	reflex update_simulation when: empty(bus where !each.has_belief(ruta_finalizada)) {     	
		...
		save skipped_bus_stops to: "analysis/experimental_tests/" + prefix + "/" + n_simulaciones + "_" + prefix + "_bus_skipped_stops.csv"; 
		save logs_ruta_dinamicas to: "analysis/experimental_tests/" + prefix + "/" + n_simulaciones + "_" + prefix + "_bus_dynamic_stops.csv"; 
...
species bus_stop skills: [fipa] {
	...
	int umbral_saturacion <- 35; // Umbral de saturación de una parada.
		...
		if content[0] = "¿Quieres subir?" {
			...
			if destino[0] in content[1]{
				...
				if !linea_directa_disponible {
				    ...
				    // Encontrar todas las líneas que pasan por el destino
